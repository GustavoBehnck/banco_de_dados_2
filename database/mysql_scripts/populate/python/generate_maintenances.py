...
nome_tabela = 'maintenances'
nome_do_arquivo_final = 'populate_' + nome_tabela
extensao_do_arquivo = '.sql'

pecas_tratores = ["Motor", "Transmissão", "Caixa de câmbio", "Sistema hidráulico", "Bomba hidráulica", "Braço hidráulico", "Pistões hidráulicos", "Sensores de proximidade", "GPS/GNSS", "Controlador eletrônico (ECU)", "Bateria", "Sistema de carregamento da bateria", "Atuadores de direção", "Sistema de freios", "Engate de implementos", "Esteiras/Rodas", "Lâminas de corte ou implementos de tração", "Cabos e chicotes elétricos", "Sistema de telemetria", "Radiadores / Sistema de arrefecimento"]
jira_link_base = f"https://graomestre.atlassian.net/browse/TRACTOR_MAINT/"
...
def random_timestamp_string():
    ...
    delta_total = int((end - start).total_seconds())
    random_seconds = random.randint(0, delta_total)
...
# ----------------------------------------------------------------------------------------------------
linhas_sql = [f'USE {nome_banco};', f'INSERT INTO {nome_tabela} (vehicle_id, date, reason, link_to_ticket, created_at, updated_at) VALUES']
...
for i in range(1000):
    ...
    date, limbo = random_timestamp()

    reason = "Dano em: " + random.choice(pecas_tratores)

    link_to_ticket = jira_link_base + random_timestamp_string()

    created_at, updated_at = random_timestamp()

    nova_linha = f"('{vehicle_id}', '{date}', '{reason}', '{link_to_ticket}', '{created_at}', '{updated_at}'),"

...
nome_tabela = 'vehicles_contracts'
nome_do_arquivo_final = 'populate_' + nome_tabela
extensao_do_arquivo = '.sql'

observacoes = [
    "'Pneus apresentam desgaste moderado, porém dentro dos limites aceitáveis para operação.'",
    "'Horímetro indica uso compatível com o período contratado, sem sinais de manipulação.'",
    "'Sistema hidráulico demonstra pequenos vazamentos que não comprometem a função, mas exigem monitoramento.'",
    "'Lataria possui arranhões leves decorrentes de uso normal em campo, sem danos estruturais.'",
    "NULL",
    "NULL",
    "NULL",
    "NULL",
...
# ----------------------------------------------------------------------------------------------------
linhas_sql = [f'USE {nome_banco};', f'INSERT INTO {nome_tabela} (contract_id, vehicle_id, observation, created_at, updated_at) VALUES']
...
for i in range(1000):
    # CLIENT ID
    while True:
        contract_id = random.randint(1, 1000)
        if contract_id not in used_contract_ids:
            used_contract_ids.append(contract_id)
            break
    ...
    nova_linha = f"('{contract_id}', '{vehicle_id}', {observation}, '{created_at}', '{updated_at}'),"
